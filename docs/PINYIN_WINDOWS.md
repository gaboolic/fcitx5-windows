...
**fcitx5-chinese-addons** 请自行克隆到默认 **`../fcitx5-chinese-addons`**（与 `CHINESE_SRC` 一致），或设置 **`CHINESE_SRC`** 指向已有源码；版本需与 libime / fcitx5 对齐（见各仓库 `CMakeLists.txt`）。
...
也可一步完成 (4)+(5)：**`.\scripts\install-fcitx5-ime.ps1 -Stage …\stage -DeployDir C:\Fcitx5Portable`**。

## 2. 安装本仓库（fcitx5 核心 + 模块）到统一 prefix

等价于步骤 (1)；若不用 PowerShell 脚本，可手动：
...
- `stage/lib/cmake/Fcitx5Core/`
- `stage/lib/cmake/Fcitx5Module/` 以及各 `Fcitx5Module*/`
- `stage/lib/fcitx5/*.dll`（已有模块）
- `stage/share/fcitx5/`

## 3. 构建并安装 libime 到同一 prefix

```bash
export STAGE=/path/to/fcitx5-windows/stage
...
完成后 `LibIMEPinyin` / `LibIMETable` 的 CMake 包应在 `stage/lib/cmake/` 下。
...
**处理**：本仓库已在 TSF / `Fcitx5.exe` 启动时设置 **`LIBIME_MODEL_DIRS`** 指向安装根下的 **`lib/libime`**，并在 **`scripts/02-build-deps.sh`** 里对上游 **`languagemodel.cpp`** 打补丁：在 **`_WIN32`** 下用 **`;`** 分隔 `LIBIME_MODEL_DIRS`（上游用 **`:`** 会把 `C:\...` 拆坏）。重新走完整 stage + 安装包流程后应恢复。若你自行编 libime 而未用该脚本，需同步补丁或等上游修复。  
这些路径必须用 **UTF-8** 传给 `setEnvironment`（安装根来自宽字符 `GetModuleFileNameW` 时不能用 `path::string()` 的 ACP 窄串），否则 `FCITX_ADDON_DIRS` / `LIBIME_MODEL_DIRS` 会错，表现为整句排序乱或插件根本加载失败。
...
1. 将 **`stage`**（或任意完整 prefix）当作安装根：保证 **`bin`** 含 IME DLL（`fcitx5-x86_64.dll` 或 MinGW 的 `libfcitx5-x86_64.dll`）、`Fcitx5Core.dll`、`Fcitx5Utils.dll`、可选 **`penguin.ico`**；**`lib/fcitx5`** 含拼音及依赖 addon；**`share/fcitx5`** 完整。
2. **安装 / 卸载**（**管理员** PowerShell；`DllRegisterServer` 写 **HKCR**）。**`install-fcitx5-ime.ps1`** 会依次执行 **`04-deploy-to-portable.ps1`** 与 **`05-register-ime.ps1`**：

   ```powershell
   .\scripts\install-fcitx5-ime.ps1 -Stage D:\path\to\stage -DeployDir C:\Fcitx5Portable
   ...
   - 在 **Git Bash / MSYS** 中可：`powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./scripts/install-fcitx5-ime.ps1 …`（参数与上相同；卸载同理换 **`uninstall-fcitx5-ime.ps1`**）
...
- `cmake --install` 时若出现部分路径落到 **`C:/share/...`**，与根目录 `CMAKE_INSTALL_PREFIX` 设为 `C:` 有关；建议使用 **`--prefix` 指向显式目录**（如上面的 `stage`）。
