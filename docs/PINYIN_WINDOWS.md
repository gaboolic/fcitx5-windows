...
**处理**：本仓库已在 TSF / `Fcitx5.exe` 启动时设置 **`LIBIME_MODEL_DIRS`** 指向安装根下的 **`lib/libime`**，避免安装包内嵌的绝对路径在用户机器上失效。若 **`LIBIME_MODEL_DIRS` 含多个 Windows 路径**，上游 libime 若仅用 **`:`** 分隔，会把 `C:\...` 拆坏，需在 **libime** 侧自行支持 **`;`** 或单一路径，或关注上游修复。  
